...
function validateScoringRequest(payload: unknown): { request?: ValidatedScoringRequest; error?: string } {
  ...
  if (typeof body.simulationLevel !== "number" || ![1, 2, 3].includes(body.simulationLevel)) {
    return { error: "simulationLevel must be 1, 2, or 3" };
  }
  ...
  return {
    request: {
      userID: body.userID,
      simulationLevel: body.simulationLevel,
      conversationTurns,
...
export const handler: APIGatewayProxyHandler = async (event) => {
  ...
  const scenario = mapScenarioFromLevel(request.simulationLevel);
  const promptVersion = `${scenario}-scoring-v1`;
  const scoringPrompt = SCORING_PROMPTS[scenario];
  if (!scoringPrompt) {
    return respond(HTTP_STATUS.INTERNAL_SERVER_ERROR, {
      error: "Configuration error",
      requestId,
