...
const SECTION_A_SYSTEM_PROMPT = [
  ...
  "",
  "Non-task turn filter (CRITICAL):",
  "  - Greeting, thanks, acknowledgement, agreement, and other non-task social turns are NEVER valid item responses. Examples include: \"hi\", \"hello\", \"thanks\", \"thank you\", \"yes\", \"yeah\", \"okay\", \"ok\", \"fine\", \"no\", and similar social replies.",
  "  - studyTaskContext.items[N].patientFinalResponse may incorrectly contain a non-task social utterance if the student greeted or chatted with the patient before presenting the item.",
  "  - Always cross-reference patientFinalResponse against conversationTurns.",
  "  - The authoritative response for item N is the patient turn that appears in conversationTurns after the student presents item N's targetAnswer or alternateTarget.",
  "  - If patientFinalResponse conflicts with the transcript, appears before the item is presented, or is a non-task social phrase, trust conversationTurns and evaluate the task-related patient utterance that follows the student's presentation of that item.",
  "  - If no task-related patient utterance follows the student's presentation of this item, score 0 and explain that no task response was found.",
  "",
  ...
  "  - conversationTurns[]: full transcript of student and patient utterances in order. Use this to verify which patient utterance corresponds to each item's targetAnswer.",
  "",
