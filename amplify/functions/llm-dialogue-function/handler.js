...
import { createResponse, parseJsonBody, HTTP_STATUS, buildCorsHeaders, callOpenAIChat, OpenAIUpstreamError, requireRuntimeTokenClaims, applyRuntimeClaimsToBody, RuntimeTokenError, putItem, generateTimestamp, queryItems, resolveRuntimeConfig, ContextResolutionError, createDynamoDbClient, } from "../shared";
...
function parseStructuredDialogue(content) {
...
function validateDialogueRequest(payload) {
...
function buildSystemPrompt(baseInstructions) {
    return [
        baseInstructions,
...
export const handler = async (event) => {
    ...
    const respond = (statusCode, body) => createResponse(statusCode, body, corsHeaders);
    ...
    if (event.httpMethod !== "POST") {
        return createResponse(HTTP_STATUS.METHOD_NOT_ALLOWED, { error: "Method not allowed", requestId, retryable: false }, { ...corsHeaders, Allow: "GET,POST,OPTIONS" });
    }
    if (!OPENAI_API_KEY) {
    ...
    try {
        ...
        payload = applyRuntimeClaimsToBody(payload ?? {}, runtimeClaims);
    }
    catch (error) {
    ...
    let runtimeConfig;
    try {
        runtimeConfig = await resolveRuntimeConfig({ context: request.context }, ASSIGNMENT_TABLE_NAME || "", SCENE_CATALOG_TABLE_NAME || "", PATIENT_PROFILE_TABLE_NAME || "", dynamo);
    ...
    const totalInputChars = historyMessages.reduce((sum, message) => sum + message.content.length, 0);
    ...
    const temperature = request.options.temperature
        ...
        ?? DEFAULT_TEMPERATURE;
    const maxOutputTokens = Math.floor(request.options.maxOutputTokens
    ...
    const invokeDialogueModel = async (strictMode) => {
    ...
    if (request.context?.sessionId && TURN_TABLE_NAME && lastUserMessage) {
        try {
            ...
            if (typeof turnIndex !== "number") {
                const existingTurns = await queryItems(TURN_TABLE_NAME, "sessionId = :sid", { ":sid": request.context.sessionId }, dynamo, { scanIndexForward: false, limit: 1 });
                turnIndex = existingTurns[0]?.turnIndex ? Number(existingTurns[0].turnIndex) + 1 : 1;
            }
            await putItem(TURN_TABLE_NAME, {
